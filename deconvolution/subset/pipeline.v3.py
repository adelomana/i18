# define cell ids, build recursive bam files, make sure you recapitulate all mutations incrementionally

# define barcodes and make sure they are unique across wells
...
def cell_analyzer(argument):
    ...
    i=argument[0]
    output_dir=argument[1]

    printt('working with {}/{} cells'.format(i+1,len(cells)))
    working_cells=cells[:i+1]
    target_dir=output_dir+'merged.{:03d}.cells'.format(i+1)
    ...
    command='freebayes --fasta-reference {} {} --min-alternate-count 3 --min-alternate-fraction 0.75 --skip-coverage 200 --min-mapping-quality 30 --min-base-quality 20 > {}'.format(fasta_reference_file,one_cell_bam_out_file,target_vcf_file)
    printt(command)
    os.system(command)
    target_vcf_file=target_dir+'/aggregated_variants.vcf'
    command='freebayes --fasta-reference {} {} --min-alternate-count 3 --min-mapping-quality 30 --min-base-quality 20 > {}'.format(fasta_reference_file,aggregated_bam_out_file,target_vcf_file)
    printt(command)
    os.system(command)
...
def job_distributor(cells,label,path_to_bamfile):
    ...
    # f.1. define top outputdir
    output_dir='/Volumes/omics4tb2/alomana/projects/i18/results/deconvolution/subset/{}/'.format(label)
    if os.path.exists(output_dir) == False:
        os.mkdir(output_dir)

    cell_indexes=numpy.arange(len(cells))
    ###cell_indexes=cell_indexes[:8] ####################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    arguments=[[cell_index,output_dir] for cell_index in cell_indexes]
